...
import { Bell, Clock, UserPlus, AlertTriangle } from "lucide-react";
import Link from "next/link";
import { useEffect, useMemo, useRef, useState } from "react";
import { useAuth } from "@/lib/auth";
import { buildNotifications, type PendingAccount } from "@/lib/notifications";
...
export function NotificationBell() {
  ...
  const items = useMemo(
    ...
    [user?.isManagement, user?.collaboratorId, state.projects, state.tasks, pendingUsers],
  );
        ...
        <div className="absolute right-0 z-50 mt-1 w-[min(calc(100vw-2rem),360px)] overflow-hidden rounded-xl border border-line bg-surface shadow-lg">
          ...
          <div className="scrollbar-minimal max-h-80 overflow-y-auto">
            {items.length === 0 ? (
              ...
              items.map((item) => {
                const Icon =
                  item.id.startsWith("pending:") ? UserPlus : item.tone === "danger" ? AlertTriangle : Clock;
                return (
                  <Link
                    key={item.id}
                    href={item.href}
